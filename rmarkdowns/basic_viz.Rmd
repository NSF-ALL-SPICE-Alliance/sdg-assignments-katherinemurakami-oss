...
```
```{r}
ggplot(data = sdr_data, aes(x= goal_8_score, y = goal_9_score, color = goal_7_score)) +
  geom_point()
```
Interpretation: Countries that have the lowed SDG goal 7 score concurrently exhibit a lower SDG goal 9 score, exhibited by the darker blue points closer to y=0.  Comparatively, dots become progressivly light blue, showcasing higher goal 7 scores,  when they have a higher goal 8 score; however, there is an evidently stronger correlation between goal 7 and goal 9 scores for countries, as its proportional trend ha fewer outliers.  There are several countries that remian outliers.
...
```
```{r}
ggplot(mena_sdr_data, aes(x= goal_8_score, y = country)) +
  geom_bar(stat = "identity") 
```
...
```{r}
ggplot(sdr_data, aes(x = goal_8_score, fill=regions_used_for_the_sdr)) + 
  geom_histogram(bins=33, color="black")
```
Interpretation: Countries within the OECD region display a higher goal 8 score, whilst those in the Sub-Saharan Africa region have the lowest; however, there are outlier countries and residual data points for each region.
